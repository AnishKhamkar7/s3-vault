...
const ACCESS_TOKEN_EXPIRY = 30 * 30 * 60 * 1000; // 30 Minutes in seconds
const REFRESH_TOKEN_EXPIRY = 60 * 10000; // 7 Days in seconds
...
class SessionManager {
  async read(token: string): Promise<SessionType> {
    return new Promise((res, rej) => {
      ...
      jwt.verify(token, env.JWT_ACCESS_SECRET, async function (err, payload) {
        ...
        if (!_session || _session === {}) return rej('Invalid');
  ...
  async create(user: SessionUserType) {
    ...
    console.log('Creating session for SID:', sid);
    await rc.set(`sess:${sid}`, JSON.stringify(sp), { EX: REFRESH_TOKEN_EXPIRY });
    console.log('Session created in Redis for SID:', sid);
  ...
  async refresh(ref: string) {
    ...
    const _session = await rc.get(`sess:${decoded.sid}`);
